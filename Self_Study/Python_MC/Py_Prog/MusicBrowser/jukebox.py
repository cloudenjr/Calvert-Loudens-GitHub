...
# CHALLENGE:
#
# Fix the DataListBox class so that it displays the songs for the correct album, when the same album name appears
# more than once in the database.
#
# HINT:
# It's worth take the time to work out what you need the query to be, then check the code to see at what point you
# actually have the information you need.
# ******************************************************************************************************************
...
class DataListBox(Scrollbox):
    ...
    def __init__(self, window, connection, table, field, sort_order=(), **kwargs):
        ...
        # artistList.bind('<<ListboxSelect>>', on_select)
    ...
    def requery(self, link_value=None):
        ...
        for value in self.cursor:
            self.insert(tkinter.END, value[0])  # here we get a tuple representing each row
...
if __name__ == '__main__':
    ...
    mainWindow.columnconfigure(3, weight=1)   # spacer coloum on right
    ...
    # =========  Artists Listbox ============
    # ==== Created a DataListBox Class, so this call to Scrollbox is no longer needed. ======
    # artistList = Scrollbox(mainWindow) # to add color to background column, just add
                                         # 'background="color"' to Scrollbox param
    ...
    # We can delete the below code because we've implemented this in our scrollbox class
    #
    # artistScroll = tkinter.Scrollbar(mainWindow, orient=tkinter.VERTICAL, command=artistList.yview)
    # artistScroll.grid(row=1, column=0, sticky='nsew', rowspan=2)
    ...
    # ==== Binding Events To Functions, So That The Function Is Called When The Event Occurs ========
    ...
    # albumScroll.grid(row=1, column=1, sticky='nsew', rowspan=2)
